...
AUTOPLAY_MIN_INTERVAL_SECONDS=15
...
`DEFAULT_AUTO_TRACKS`와 `MAX_AUTO_TRACKS`는 전용 채널의 `auto:` 요청으로 관련 곡을 추가할 때의 기본/최대 개수입니다. `AUTOPLAY_REFILL_CANDIDATES`는 재생 중 백그라운드 보충이 한 후보 목록에서 점수화할 기본 결과 수이며 기본값은 10입니다. 직전 배치의 유효 후보 중 최근곡 비율이 30% 이상이면 다음 조회만 12개, 60% 이상이면 15개로 늘리고, 비율이 내려가면 다시 기본값으로 돌아갑니다. 한 리필에서 성공한 Radio 조회는 한 번만 하므로 후보가 한 곡뿐이어도 즉시 두 번째 조회를 만들지 않습니다. 후보는 Radio 순서, 길이, 풀·숏·게임·커버·반복 표시, 공식 영상 여부와 실제 라이브 상태를 합산해 정렬하며, 수동 `autoN:` 관련 곡에도 같은 품질 점수를 적용합니다. Cover와 녹화된 Live 버전은 기본 -80점이지만 검색어나 현재 시드가 같은 스타일이면 그 스타일 벌점만 0점이 됩니다. 실제 진행·예정·종료 방송의 -200점은 Live 스타일을 원해도 유지합니다. 최저점 컷은 없어 낮은 점수를 이유로 다시 조회하지 않습니다. 유효한 YouTube Radio 조회가 비거나 실패해도 같은 시도에서 텍스트 검색을 추가하지 않습니다. 실패하거나 대기곡도 없으면 기존 백오프를 적용하고, 대기곡이 있으면 더 보수적으로 재생 시드가 바뀔 때까지 기다립니다. 영상 ID가 없어 Radio를 만들 수 없을 때만 fallback 검색을 한 번 사용합니다. 한 번 조회한 목록에서 선별한 곡 중 최대 6곡은 길드별 메모리 후보 풀에 보관하고 다음 자동재생 보충에 먼저 재사용하므로, 한 조회로 최대 7곡을 사용합니다. 사용자가 직접 입력한 `auto25:` 같은 요청에는 후보 풀을 적용하지 않습니다. 수동 신청이나 시드 변경으로 사용하지 않은 풀을 버릴 때는 최근곡 밴에 넣지 않습니다. 자동재생 중복 방지는 최근 최대 50곡을 기억하며, 각 기록은 `AUTOPLAY_HISTORY_TTL_SECONDS`(기본 43200초, 12시간)가 지나거나 50곡 한도에서 먼저 밀려나면 다음 후보 검사 때 만료됩니다.

자동재생 검색·선택 기록은 기본적으로 `logs/autoplay.jsonl`에 UTF-8 JSONL(한 줄에 JSON 하나)로 저장합니다. `LOG_LEVEL`과 별개로 동작하며 `AUTOPLAY_LOG_ENABLED=false`로 끌 수 있습니다. `AUTOPLAY_LOG_FILE`로 경로를 지정하고, `AUTOPLAY_LOG_MAX_BYTES`(기본 5 MiB)와 `AUTOPLAY_LOG_BACKUP_COUNT`(기본 3개)로 순환 보관합니다. 기본 보관량은 현재 파일과 `.1`~`.3`을 합쳐 약 20 MiB이며, 오래된 파일은 순환 시 교체됩니다. 로그를 장기 비교할 때는 순환되기 전에 별도로 보관하세요.

- `search_results`: 검색 ID, 시드, Radio/검색 주소, 스타일 의도, 후보 전체의 원래 순위·품질 총점·점수 계산용 메타데이터와 추출 결과(`candidate`, `seed_or_duplicate`, `invalid_video_id`, `candidate_limit`). 수동 `autoN:`도 포함합니다.
- `selection`: 서버 ID, 검색/캐시 구분, 각 후보의 최근 재생 여부·마지막 재생 후 경과 초·적용 감점·선택 점수와 결과(`queued`, `cached`, `hard_excluded`, `recent_requires_search`, `not_selected`). `search_id`와 `track_id`로 원래 검색 결과와 연결합니다. 새 곡 우선 정책이므로 선택 점수만으로 최종 순서를 해석하지 않습니다.

- `search_failed`: 백그라운드 검색 오류 종류와 다음 재시도 대기 시간.

점수 조정 시 `selection`에서 선택된 곡과 밀린 후보를 비교하고, 같은 `search_id`의 `search_results`에서 제목·길이·채널·방송 상태·스타일을 확인하면 됩니다. `schema_version`과 `policy_version`도 기록합니다. 이 로그는 대기열 선택 기록이며 실제 청취 완료 기록은 아닙니다. 요청자 정보, 인증 정보와 재생용 스트림 URL은 저장하지 않습니다. 파일 기록 실패로 재생을 중단하지 않습니다.
...
`/setupmusic`을 실행하면 전용 채널에 컨트롤 패널이 하나 만들어지고 봇을 재시작해도 같은 메시지를 다시 사용합니다. 저장된 메시지 ID가 유실된 경우에는 채널 기록에서 기존 패널을 복구합니다. 봇이 시작될 때는 가장 최신 패널 하나만 남기고 전용 채널의 다른 모든 메시지를 삭제합니다. 재생 중에는 다음 곡과 조작 버튼이 표시되고, 모든 곡이 끝나거나 정지하면 패널을 삭제하는 대신 재생 버튼이 비활성화된 “재생 대기 중” 상태로 돌아갑니다. 자동재생과 최근 재생 버튼은 대기 중에도 사용할 수 있습니다. `최근 재생` 버튼은 현재 실행 중 실제 재생을 시작한 이력을 최신순으로 개인에게만 보여 줍니다. 자동재생 중복 방지와 같은 보관 기간(기본 12시간) 및 최대 50회 제한을 사용하며, 같은 곡을 다시 재생해도 매번 별도 항목으로 표시합니다. 건너뛰기만 한 자동재생 후보와 신청자 정보는 표시하지 않고, 메모리 기록이므로 봇을 재시작하면 초기화됩니다. 화면 목록과 별개로 자동재생 중복 방지 기록은 같은 곡의 최신 시각 하나만 유지합니다. 자동재생을 켜면 현재 흐름의 마지막 곡을 `auto:`와 같은 방식으로 검색하고 대기열이 1곡 이하일 때마다 한 곡을 추가합니다. 현재 곡·대기열·검색 기준곡은 점수와 무관하게 절대 제외합니다. 최근 재생 이력에 없는 곡을 품질 점수와 무관하게 최근곡보다 먼저 선택하며, 새 곡끼리는 품질 점수가 높은 곡을 우선합니다. 캐시에 최근곡만 남으면 이를 즉시 재사용하지 않고 현재 시드로 새 후보를 조회합니다. 새 조회에도 최근곡만 있으면 가장 오래전에 재생한 곡을 선택해 재생을 이어갑니다. 최근곡의 마지막 재생 시각이 같으면 품질 점수를 비교합니다. 직전 배치의 최근곡 비율에 따른 -80/-40/-10점은 선택 점수 기록에 유지되지만 새 곡 우선순위를 뒤집지는 않습니다. 영상 ID가 달라도 아티스트와 곡명이 같은 MV, 공식 음원, 가사 영상은 같은 곡으로 보고, Live, Remix, Cover는 별도 버전으로 유지합니다. 안전하게 사용할 후보가 없지만 다음 곡이 이미 대기 중이면 외부 검색 없이 재생 기준이 바뀔 때까지 기다렸다가 새 기준으로 다시 보충하고, 대기곡도 없거나 검색이 일시적으로 실패하면 자동재생을 끄거나 음성 채널에서 나갈 때까지 간격을 두고 다시 시도합니다.
