...
- `/auto n:<개수> 곡명:<검색어>`로 기준 곡을 포함한 관련 곡 여러 개 추가
...
- 일반 곡 검색과 추가는 전용 채널 메시지로 처리하고, 관련곡 묶음은 `/auto`로 신청
- `/remove` 번호로 대기열 곡 삭제
- `/leave` 음성 채널 퇴장
...
`MUSIC_FEEDBACK_DELETE_SECONDS=10`이면 메시지 또는 `/auto`로 곡을 신청했을 때 나오는 임시 추가 확인을 10초 뒤 삭제합니다. 다른 슬래시 명령어와 버튼의 일반 개인 응답은 신청자에게만 보이며 `EPHEMERAL_RESPONSE_DELETE_SECONDS=15`초 뒤 정리됩니다. 대기열 관리 메시지는 곡을 삭제할 때마다 만료 시간이 다시 계산되고, 마지막 삭제로부터 `QUEUE_DELETE_RESPONSE_DELETE_SECONDS=30`초 뒤 삭제됩니다.
...
`MAX_AUTO_TRACKS`는 `/auto`의 필수 `n` 옵션으로 지정할 수 있는 최대 곡 수입니다. `n`은 기준 곡을 포함한 총 곡 수입니다. `AUTOPLAY_REFILL_CANDIDATES`는 재생 중 백그라운드 보충이 한 번에 확인할 후보 수이며 기본값은 5입니다.
