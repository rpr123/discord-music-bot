...
class MusicNamuWikiReadingTests(unittest.TestCase):
    def test_bot_reexports_moved_namuwiki_reading_names(self) -> None:
        for name in READING_MOVED_NAMES:
            with self.subTest(name=name):
                self.assertIs(
                    getattr(bot, name),
                    getattr(music_namuwiki_parsing, name),
    ...
    def test_groups_original_lyrics_and_annotated_reading_are_extracted(self) -> None:
        ...
        self.assertEqual(
            music_namuwiki_parsing.split_namuwiki_lyrics_groups(lyrics),
            [
                ["君の声", "きみのこえ", "너의 목소리"],
                ["空へ", "そらへ", "하늘로"],
            ],
        ...
        self.assertEqual(
            music_namuwiki_parsing.extract_namuwiki_original_lyrics(lyrics),
            "君の声\n空へ",
        )
        self.assertEqual(
            music_namuwiki_parsing.extract_namuwiki_annotated_reading(lyrics),
            "君(きみ)の声(こえ)\n空(そら)へ",
        )

    def test_incomplete_groups_do_not_produce_annotated_reading(self) -> None:
        self.assertIsNone(
            music_namuwiki_parsing.extract_namuwiki_annotated_reading(
                "君の声\n너의 목소리"
            )
        )
    ...
    def test_source_prefers_japanese_lyrics_then_namuwiki_original(self) -> None:
        ...
        self.assertEqual(
            music_namuwiki_parsing.get_hiragana_reading_source_lyrics(
                track,
                "直接の歌詞",
            ),
            "直接の歌詞",
